...
export class EnhancedDocumentParser {
  ...
  private async extractDocumentMeta(artifact: DocumentArtifact): Promise<DocumentMeta> {
    ...
    if (artifact.docType === 'EOB') {
      ...
      meta.totals = {
        billed: this.generateMockAmount(5000, 50000),
        allowed: this.generateMockAmount(3000, 40000),
        planPaid: this.generateMockAmount(2000, 30000),
        patientResponsibility: this.generateMockAmount(500, 5000)
      };
      meta.appeal = {
      ...
      meta.totals = {
        billed: this.generateMockAmount(5000, 50000),
        patientResponsibility: this.generateMockAmount(1000, 10000)
      };
    }
  ...
  private async extractLineItems(artifact: DocumentArtifact): Promise<LineItem[]> {
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      const charge = this.generateMockAmount(100, 5000);
  ...
  private generateMockNPI(): string {
    return '1' + Math.random().toString().slice(2, 11).padEnd(9, '0');
  }

  private generateMockTIN(): string {
    return Math.random().toString().slice(2, 11).padEnd(9, '0');
  }

  private generateMockClaimId(): string {
    return 'CLM' + Math.random().toString(36).substr(2, 12).toUpperCase();
  }

  private generateMockAccountId(): string {
    return 'ACC' + Math.random().toString(36).substr(2, 10).toUpperCase();
  }
  ...
  private generateMockAmount(min: number, max: number): MoneyCents {
  ...
  private generateMockCPTCode(): { system: "CPT" | "HCPCS"; value: string } {
    ...
    if (Math.random() < 0.8) {
      return { system: 'CPT', value: cptCodes[Math.floor(Math.random() * cptCodes.length)] };
    } else {
      return { system: 'HCPCS', value: hcpcsCodes[Math.floor(Math.random() * hcpcsCodes.length)] };
    }
  }
